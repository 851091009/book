...
SQL 语句的占位符是 ?, 而MySql 的占位符是 %s，select() 函数在内部自动替换。注意要始终坚持使用带参数的 SQL，而不是自动拼接 SQL 字符串，这样可以防止 SQL 注入攻击。
...
注意到 Model 只是一个基类，如果将具体的子类如 User 的映射信息读取出来呢？ 答案就是 通过 metaclass: ModelMetaclass：
````python
